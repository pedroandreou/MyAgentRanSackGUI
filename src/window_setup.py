...
def layout():
    ...
    first_col = [[sg.Text('Folder'), sg.In(size=(25,15), enable_events=True , key='-FOLDER-'), sg.FolderBrowse(enable_events=True , key='-BROSWER-')],
                ...
                [sg.Listbox(values=[], enable_events=True, size=(40, 20), key="-FILE LIST-"), sg.Checkbox('Match case', size = (10, 5), default=False, key='-MATCHCASE-'), sg.Checkbox('Regular expression', size = (20, 5), default=False, key='-RE-'), sg.InputText(size = (20, 10), enable_events=True, key='-REINPUT-')],
